...
echo "SELECT s.store,e.after_cycle_update,e.after_cycle,e.recurring_discount_value,e.contract_id,e.store_id,e.next_billing_date,s.access_token from subscriptionOrderContract e, install s, billingAttempts as b where e.store_id = s.id AND e.next_billing_date = '$currentDate' AND  e.contract_status = 'A' AND contract_inprocess = 'no' AND e.next_billing_date NOT IN (SELECT b.renewal_date FROM billingAttempts b WHERE e.contract_id = b.contract_id AND b.status = 'Skip' AND b.renewal_date = '$currentDate') GROUP BY e.contract_id limit 0,10";
...
if ($row_count > 0) {
	...
	foreach ($row_data as $rowVal) {
		...
		if($rowVal['max_cycle'] && $plan_type == 'membership'){
            ...
            // print_r($tag_data);
				...
				try{
                    ...
                    print_r($customerUpdateExecution);
					$action = 'remove';
                    
					// correct this condiction according to need
					
					//    addRemoveCustomerTag($store, $action, $rowVal['shopify_customer_id'], $config ,$customer_tag );
		...
		if ($execute_cron == 'Yes') {
			...
			// print_r($contract_products_array);
			// die;
			...
			try {
				...
				if (!count($billingAttemptCreateApi_error)) {
					...
					$insertbillingattempt = "INSERT INTO billingAttempts (store_id,plan_type,contract_id,contract_products,billingAttemptId,billing_attempt_date,status,renewal_date,updated_at) VALUES ('$store_id','$plan_type','$contract_id','$contract_products','$billingAttempt_id','$currentDate','Pending','$currentDate','$current_date')";
