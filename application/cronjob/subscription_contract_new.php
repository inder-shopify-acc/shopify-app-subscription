...
function processContracts($processDataParameters, $contract_html, $merchant_subscription_title, $mail_count, $store_id, $customerEmail, $order_token, $contract_id, $payment_instrument_type, $customerId, $customerTagsArray, $orderTagsArray, $billingAddressAry, $shopifies, $email_template_data, $replace_with_value, $subscription_line_items)
{
    ...
    // print_r($contract_details_data);die;
    $plan_type = '';
        ...
        try {
            ...
            if (isset($contract_detail['data']['subscriptionContract']['lines']['edges'])) {
                ...
                foreach ($contract_detail['data']['subscriptionContract']['lines']['edges'] as $line) {
                    ...
                    // $purchage_plan_amount = $node['pricingPolicy']['cycleDiscounts'][1]['computedPrice']['amount'];
                    ...
                    // echo "Purchage Plan Amount: " . $purchage_plan_amount;
            ...
            if ($plan_type == 'membership' || $store == 'thediyart1.myshopify.com') {
                ...
                if (!empty($get_member_plan_handle)) {
                    // echo 'plan handle not empty<br>';
                    // $add_plan_tag = $get_member_plan_handle->unique_handle;
                    $add_plan_tag = array($get_member_plan_handle->unique_handle);
                    // array_push($customerTagsArray, $add_plan_tag);
                    // array_push($customerTagsArray, 'sd-'.$add_plan_tag);
                }
                ...
                try {
                    ...
                    // echo "tags in string = " . $mergedTagsString . "<br>";
            ...
            try {
                ...
                // print_r($get_selling_plan_query);

                $selling_plan_data = $get_selling_plan_query->fetch(PDO::FETCH_ASSOC);
                // print_r($selling_plan_data);echo($selling_plan_data['offer_trial_status']);echo($selling_plan_data['renew_on_original_date']);
                // die();
                // print_r($selling_plan_data);
                ...
                if ($selling_plan_data['offer_trial_status'] == 1) {
                    ...
                    if ($selling_plan_data['renew_on_original_date'] === 'false' || $selling_plan_data['renew_on_original_date'] === '0') {
                        ...
                        echo $renewalDate;
                        // echo($date);
                        // $date->modify('+'.$selling_plan_data['trial_period_value'].' '.$selling_plan_data['trial_period_type'].''); // Modify the date
                        // $renewalDate = $date->format('Y-m-d'); // Format back to string (e.g., 'YYYY-MM-DD')
                ...
                // echo "Purchage Plan id: " . $appSubPlanId;
                if ($appSubPlanId) {
            ...
            try {
                ...
                // $customerName and $currentUtcDate already set

                $insert = $stmt->execute();

                // }else {
                //     $customerNameEscaped = $db->quote($customerName);
                //     $sql_customer_query = "INSERT INTO customers (`store`,`store_id`, `shopify_customer_id`, `email`, `name`, `created_at`) VALUES ('$store','$store_id', '$customerId', '$customerEmail', '$customerNameEscaped', '$currentUtcDate')";
            ...
            /***** From Webhook File *****/
            // echo ' proceed contract:= ' . $proceed_contract;
            ...
            if ($proceed_contract == 'Yes') {
                ...
                try {
                    ...
                    // echo('else member email');
                    // echo 'Plan Type: ' . $plan_type . '<br>';
                    // echo 'Store: ' . $store . '<br>';
                    // echo 'Customer ID: ' . $customerId . '<br>';
                    // echo 'Selling Plan ID: ' . $selling_plan_id . '<br>';
                    // echo 'Contract ID: ' . $contract_id . '<br>';
                    // echo 'Purchase Plan Variant ID: ' . $purchage_plan_variantId . '<br>';

                    if ((!empty($email_template_data)  && $plan_type == 'membership') || (!empty($email_template_data) && $store == 'thediyart1.myshopify.com')) {

                        // print_r($purchage_plan_variantId);
                        echo "siofjdkfkdfjk";
                        send_membership_email($store, $customerId, $selling_plan_id, $contract_id, $purchage_plan_variantId, $store_install_data, $db, $SHOPIFY_DOMAIN_URL);
                    }
                } catch (Exception $e) {
...
function send_membership_email($store, $customerId, $selling_plan_id, $contract_id, $variantId, $store_install_data, $db, $SHOPIFY_DOMAIN_URL)
{
    ...
    try {
        ...
        echo $store_id;
        // $customerDetails = DB::table('customers')->where('store', '=', $store)->where('customer_id', $customerId)->firs();
        ...
        foreach ($customer_data as $value) {
            ...
            // echo 'discounted_product_collection_type---> '.$discount_on.'<br>';
            // die;
            ...
            // echo 'Custom Email HTML: ' . $custom_email_html . '<br>';
            // echo 'CCC Email: ' . $ccc_email . '<br>';
            // echo 'BCC Email: ' . $bcc_email . '<br>';
            // echo 'Reply To: ' . $reply_to . '<br>';
            // echo 'Email Template HTML: ' . $email_template_html . '<br>';

            // print_r($free_gift_uponsignupSelectedDays);
            // print_r($free_gift_checked);
            ...
            if ($free_gift_checked == 1 && $free_gift_uponsignupSelectedDays == 'Immediately_after_signup') {
                ...
                if ($data) {
                    ...
                    echo '<b><i>Successfully draft order created .....<br></i></b>';
                } else {
...
function membershipAllEmailTemplates($template_data, $db, $SHOPIFY_DOMAIN_URL)
{
    ...
    // $template_setting = DB::table($template_name)->select($template_name . '.*')->where('store', $store_name)->first();
    ...
    if (empty((array) $template_setting)) {
        ...
        // $store_details = $this->getStoreDetailsByDomain($store_name);

        $logo = $SHOPIFY_DOMAIN_URL . '/application/assets/images/emai-logo.png';

        // $shop_name = $store_details['shop_name'] ?? "";

        // $shop_email = $store_details['store_email'] ?? "";
...
function emailSend($store, $body, $subject, $customerEmail, $ccc_email, $bcc_email, $reply_to, $db, $store_id)
{
    try {
        ...
        // $storeDetails = StoreDetail::join('installs', 'store_details.store', '=', 'installs.store')
        //     ->get(['store_details.*', 'installs.access_token'])
        //     ->where('store', $store)
        //     ->first();
        ...
        // $emailConfiguration = EmailConfiguration::where('store', '=', $store)->first();
        ...
        if ($checkCondtion == 'checked') {
            ...
            // $emailEncriptionType = "tls";
            // $emailPortNumber = "587";
            // $username = "umesh@phoenixcrm.io";  // from mail
